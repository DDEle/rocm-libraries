...
v_readfirstlane_b32 s22, v[vgprSerial]             // wave 0 elects the WG's single counter writer
s_cmp_eq_u32 s22, 0                                // wave 0?
...
// loadKernArg 11 KernArgAddress dword=1 sgprOffset=0x50
// loadKernArg 12 KernArgAddress dword=1 sgprOffset=0x64
// loadKernArg 13 KernArgAddress dword=1 sgprOffset=0x58
// loadKernArg 14 KernArgAddress dword=1 sgprOffset=0x68
// loadKernArg 16 KernArgAddress dword=2 sgprOffset=0x40
s_waitcnt lgkmcnt(0)                               // wait FusedMyRank/TilesPerRank/NShard/TokenTiles/counter_ptr
s_mul_i32 s20, s[sgprWorkGroup0], 256              // n_col_base_wg = WorkGroup0 * MT0
s_mov_b32 s15, 0                                   // dst_rank = 0 (default)
s_mul_i32 s21, s13, 1                              // cand shard_lo = 1 * n_shard
s_cmp_gt_u32 s21, s20                              // shard_lo > n_col_base_wg? (WG below rank 1)
s_cbranch_scc1 label_fusedA2A_flag_skip1           // below rank 1: keep current winner
s_mov_b32 s15, 1                                   // dst_rank = 1
label_fusedA2A_flag_skip1:  /// n_col_base_wg below rank 1
s_mul_i32 s21, s13, 2                              // cand shard_lo = 2 * n_shard
s_cmp_gt_u32 s21, s20                              // shard_lo > n_col_base_wg? (WG below rank 2)
s_cbranch_scc1 label_fusedA2A_flag_skip2           // below rank 2: keep current winner
s_mov_b32 s15, 2                                   // dst_rank = 2
label_fusedA2A_flag_skip2:  /// n_col_base_wg below rank 2
s_mul_i32 s21, s13, 3                              // cand shard_lo = 3 * n_shard
s_cmp_gt_u32 s21, s20                              // shard_lo > n_col_base_wg? (WG below rank 3)
s_cbranch_scc1 label_fusedA2A_flag_skip3           // below rank 3: keep current winner
s_mov_b32 s15, 3                                   // dst_rank = 3
label_fusedA2A_flag_skip3:  /// n_col_base_wg below rank 3
s_mul_i32 s21, s13, 4                              // cand shard_lo = 4 * n_shard
s_cmp_gt_u32 s21, s20                              // shard_lo > n_col_base_wg? (WG below rank 4)
s_cbranch_scc1 label_fusedA2A_flag_skip4           // below rank 4: keep current winner
s_mov_b32 s15, 4                                   // dst_rank = 4
label_fusedA2A_flag_skip4:  /// n_col_base_wg below rank 4
s_mul_i32 s21, s13, 5                              // cand shard_lo = 5 * n_shard
s_cmp_gt_u32 s21, s20                              // shard_lo > n_col_base_wg? (WG below rank 5)
s_cbranch_scc1 label_fusedA2A_flag_skip5           // below rank 5: keep current winner
s_mov_b32 s15, 5                                   // dst_rank = 5
label_fusedA2A_flag_skip5:  /// n_col_base_wg below rank 5
s_mul_i32 s21, s13, 6                              // cand shard_lo = 6 * n_shard
s_cmp_gt_u32 s21, s20                              // shard_lo > n_col_base_wg? (WG below rank 6)
s_cbranch_scc1 label_fusedA2A_flag_skip6           // below rank 6: keep current winner
s_mov_b32 s15, 6                                   // dst_rank = 6
label_fusedA2A_flag_skip6:  /// n_col_base_wg below rank 6
s_mul_i32 s21, s13, 7                              // cand shard_lo = 7 * n_shard
s_cmp_gt_u32 s21, s20                              // shard_lo > n_col_base_wg? (WG below rank 7)
s_cbranch_scc1 label_fusedA2A_flag_skip7           // below rank 7: keep current winner
s_mov_b32 s15, 7                                   // dst_rank = 7
label_fusedA2A_flag_skip7:  /// n_col_base_wg below rank 7
s_lshl_b32 s21, s15, 3                             // dst_rank * 8 (byte offset into peer_ptr[] array)
s_add_u32 s21, s21, 0                              // kernarg offset = fusedBase + peer_ptr_0 + dst_rank*8
// loadKernArg 18 KernArgAddress dword=2 sgprOffset=s21
s_waitcnt lgkmcnt(0)                               // wait peer_ptr[dst_rank] load
s_mul_i32 s20, s15, s14                            // dst_rank * tokenTiles
s_add_u32 s20, s20, s[sgprWorkGroup1]              // counter index = dst_rank*tokenTiles + j (j = WorkGroup1)
s_lshl_b32 s20, s20, 2                             // * 4 (u32 counter byte offset)
s_add_u32 s16, s16, s20                            // counter[dst_rank][j] lo = counter_ptr + (dst_rank*tokenTiles+j)*4
s_addc_u32 s17, s17, 0                             // counter[dst_rank][j] hi (carry)
v_mov_b32 v2, s16                                  // counter addr lo -> vgpr
v_mov_b32 v3, s17                                  // counter addr hi -> vgpr
...
v_readfirstlane_b32 s20, v4                        // old -> sgpr
s_add_u32 s20, s20, 1                              // old + 1
s_cmp_eq_u32 s20, s12                              // old+1 == FusedTilesPerRank? (last WG for (dst_rank, j))
...
s_mul_i32 s20, s15, 56                             // dst_rank * sizeof(SdmaQueueDeviceHandle)
s_add_u32 s22, s22, s20                            // handle lo = FusedSdmaQueues + dst_rank*56
...
s_mul_i32 s20, s[sgprWorkGroup0], 256              // n_col_base_wg = WorkGroup0 * MT0
s_mov_b32 s21, 0                                   // dst_rank = 0 (default)
s_mov_b32 s28, 0                                   // shard_base = 0 (rank 0)
s_mul_i32 s28, s13, 1                              // cand shard_lo = 1 * n_shard
s_cmp_gt_u32 s28, s20                              // shard_lo > n_col_base_wg? (WG below rank 1)
s_cbranch_scc1 label_fusedA2A_recv_skip1           // below rank 1: keep current winner
s_mov_b32 s21, 1                                   // dst_rank = 1
label_fusedA2A_recv_skip1:  /// n_col_base_wg below rank 1
s_mul_i32 s28, s13, 2                              // cand shard_lo = 2 * n_shard
s_cmp_gt_u32 s28, s20                              // shard_lo > n_col_base_wg? (WG below rank 2)
s_cbranch_scc1 label_fusedA2A_recv_skip2           // below rank 2: keep current winner
s_mov_b32 s21, 2                                   // dst_rank = 2
label_fusedA2A_recv_skip2:  /// n_col_base_wg below rank 2
s_mul_i32 s28, s13, 3                              // cand shard_lo = 3 * n_shard
s_cmp_gt_u32 s28, s20                              // shard_lo > n_col_base_wg? (WG below rank 3)
s_cbranch_scc1 label_fusedA2A_recv_skip3           // below rank 3: keep current winner
s_mov_b32 s21, 3                                   // dst_rank = 3
label_fusedA2A_recv_skip3:  /// n_col_base_wg below rank 3
s_mul_i32 s28, s13, 4                              // cand shard_lo = 4 * n_shard
s_cmp_gt_u32 s28, s20                              // shard_lo > n_col_base_wg? (WG below rank 4)
s_cbranch_scc1 label_fusedA2A_recv_skip4           // below rank 4: keep current winner
s_mov_b32 s21, 4                                   // dst_rank = 4
label_fusedA2A_recv_skip4:  /// n_col_base_wg below rank 4
s_mul_i32 s28, s13, 5                              // cand shard_lo = 5 * n_shard
s_cmp_gt_u32 s28, s20                              // shard_lo > n_col_base_wg? (WG below rank 5)
s_cbranch_scc1 label_fusedA2A_recv_skip5           // below rank 5: keep current winner
s_mov_b32 s21, 5                                   // dst_rank = 5
label_fusedA2A_recv_skip5:  /// n_col_base_wg below rank 5
s_mul_i32 s28, s13, 6                              // cand shard_lo = 6 * n_shard
s_cmp_gt_u32 s28, s20                              // shard_lo > n_col_base_wg? (WG below rank 6)
s_cbranch_scc1 label_fusedA2A_recv_skip6           // below rank 6: keep current winner
s_mov_b32 s21, 6                                   // dst_rank = 6
label_fusedA2A_recv_skip6:  /// n_col_base_wg below rank 6
s_mul_i32 s28, s13, 7                              // cand shard_lo = 7 * n_shard
s_cmp_gt_u32 s28, s20                              // shard_lo > n_col_base_wg? (WG below rank 7)
s_cbranch_scc1 label_fusedA2A_recv_skip7           // below rank 7: keep current winner
s_mov_b32 s21, 7                                   // dst_rank = 7
label_fusedA2A_recv_skip7:  /// n_col_base_wg below rank 7
s_mul_i32 s28, s21, s13                            // shard_base = dst_rank * n_shard
s_lshl_b32 s21, s21, 3                             // dst_rank * 8 (byte offset into peer_ptr[] array)
s_add_u32 s21, s21, 0                              // kernarg offset = fusedBase + peer_ptr_0 + dst_rank*8
// loadKernArg 26 KernArgAddress dword=2 sgprOffset=s21
s_waitcnt lgkmcnt(0)                               // wait peer_ptr[dst_rank] load
...
s_mul_i32 s20, s15, s13                            // src_x = p * nShard (folded into the base, not a field)
...
s_add_u32 s34, s34, s20                            // + p*nShard (feature offset)
...
s_mul_i32 s20, s11, s[sgprSizesFree+1]             // myRank * N
s_add_u32 s20, s20, s28                            // dst row = myRank*N + j*MT1 (folded, not a field)
s_mul_hi_u32 s35, s20, s13                         // dst row offset = dst row * nShard (64-bit: unbounded in W and N) (hi)
s_mul_i32 s34, s20, s13                            // dst row offset = dst row * nShard (64-bit: unbounded in W and N) (lo)
...
s_mul_i32 s30, s13, 256                            // dst_slice = MT1 * nShard (one band's plane)
...
s_lshr_b32 s20, s[sgprStrideD1J], 3                // SUBWIN DW4: src_pitch-1 (bf16 elems -> packet elems)
s_sub_u32 s20, s20, 1                              // SUBWIN DW4: src_pitch-1 (pitch - 1)
s_lshl_b32 s20, s20, 13                            // SUBWIN DW4: src_pitch-1 (<< 13)
v_mov_b32 v9, s20                                  // SUBWIN DW4: src_pitch-1
s_lshr_b32 s20, s29, 3                             // SUBWIN DW5: src_slice-1 (bf16 elems -> packet elems)
s_sub_u32 s20, s20, 1                              // SUBWIN DW5: src_slice-1 (slice - 1)
v_mov_b32 v10, s20                                 // SUBWIN DW5: src_slice-1
...
s_lshr_b32 s20, s13, 3                             // SUBWIN DW9: dst_pitch-1 (bf16 elems -> packet elems)
s_sub_u32 s20, s20, 1                              // SUBWIN DW9: dst_pitch-1 (pitch - 1)
s_lshl_b32 s20, s20, 13                            // SUBWIN DW9: dst_pitch-1 (<< 13)
v_mov_b32 v14, s20                                 // SUBWIN DW9: dst_pitch-1
s_lshr_b32 s20, s30, 3                             // SUBWIN DW10: dst_slice-1 (bf16 elems -> packet elems)
s_sub_u32 s20, s20, 1                              // SUBWIN DW10: dst_slice-1 (slice - 1)
v_mov_b32 v15, s20                                 // SUBWIN DW10: dst_slice-1
s_lshr_b32 s20, s13, 3                             // SUBWIN DW11: rect_x-1|rect_y-1 (rectX) (bf16 elems -> packet elems)
s_sub_u32 s20, s20, 1                              // SUBWIN DW11: rect_x-1|rect_y-1 (rectX - 1)
s_sub_u32 s21, s31, 1                              // SUBWIN DW11: rect_x-1|rect_y-1 (rectY - 1, rows: NOT scaled)
s_lshl_b32 s21, s21, 16                            // SUBWIN DW11: rect_x-1|rect_y-1 ((rectY-1) << 16)
s_or_b32 s20, s20, s21                             // SUBWIN DW11: rect_x-1|rect_y-1 | (rectY-1) << 16
v_mov_b32 v16, s20                                 // SUBWIN DW11: rect_x-1|rect_y-1
v_mov_b32 v17, 0x0                                 // SUBWIN DW12: rect_z=0, default cache/swizzle
s_lshl_b32 s20, s11, 2                             // myRank * 4 (u32 flag-slot byte offset: the ATOMIC is an ADD_RTN_32)
s_add_u32 s34, s18, s20                            // flag addr lo = peer_ptr[p] + myRank*4
s_addc_u32 s35, s19, 0                             // flag addr hi (carry)
...
v_mov_b32 v25, 0x0                                 // ATOMIC DW7: loop_interval=0
  s_load_dwordx2 s[32:33], s[22:23], 0x20
s_waitcnt lgkmcnt(0)                               // wait cachedWptr pointer load
label_sdma_reserve_loop:  /// ReserveQueueSpace: CAS retry loop
v_mov_b32 v26, s32                                 // cachedWptr addr lo
v_mov_b32 v27, s33                                 // cachedWptr addr hi
...
s_mov_b32 s30, 0                                   // offset = 0 (no pad)
s_and_b32 s31, s26, 262143                         // WrapIntoRing(cur)
s_add_u32 s38, s31, 84                             // WrapIntoRing(cur) + size
s_cmp_lt_u32 s38, 262145                           // wrap+size <= queueSize? (fits without wrap)
s_cbranch_scc1 label_sdma_reserve_nopad            // fits -> no padding
s_sub_u32 s30, 262144, s31                         // offset = queueSize - WrapIntoRing(cur) (pad ring tail)
label_sdma_reserve_nopad:  /// ReserveQueueSpace: no wrap padding
s_add_u32 s34, s26, 84                             // new lo = cur + size
s_addc_u32 s35, s27, 0                             // new hi (carry)
s_add_u32 s34, s34, s30                            // new lo += offset
s_addc_u32 s35, s35, 0                             // new hi (carry)
s_mov_b32 s36, 0                                   // CanWriteUpto = false (default)
s_sub_u32 s38, s34, s24                            // CanWriteUpto: upto - cachedHwReadIndex (lo)
s_subb_u32 s39, s35, s25                           // CanWriteUpto: upto - cachedHwReadIndex (hi, borrow)
s_cmp_eq_u32 s39, 0                                // diff hi == 0? (gap < 2^32)
s_cbranch_scc0 label_sdma_canwrite_full            // hi != 0 -> gap huge -> full path
s_cmp_lt_u32 s38, 262144                           // diff < queueSize? (fast-path room check)
...
label_sdma_canwrite_full:  /// CanWriteUpto: cache says full -> read rptr
  s_load_dwordx2 s[40:41], s[22:23], 0x8
s_waitcnt lgkmcnt(0)                               // wait rptr pointer load
v_mov_b32 v32, s40                                 // rptr addr lo
v_mov_b32 v33, s41                                 // rptr addr hi
...
s_sub_u32 s38, s34, s24                            // CanWriteUpto: upto - refreshed rptr (lo)
s_subb_u32 s39, s35, s25                           // CanWriteUpto: upto - refreshed rptr (hi, borrow)
s_cmp_eq_u32 s39, 0                                // diff hi == 0?
s_cbranch_scc0 label_sdma_canwrite_done            // hi != 0 -> full (result already defaulted to 0)
s_cmp_lt_u32 s38, 262144                           // diff < queueSize?
s_cbranch_scc1 label_sdma_canwrite_ok              // room after refresh
s_mov_b32 s36, 0                                   // CanWriteUpto = false (full)
s_branch label_sdma_canwrite_done                  // -> done
label_sdma_canwrite_ok:  /// CanWriteUpto: room in ring
s_mov_b32 s36, 1                                   // CanWriteUpto = true (room)
label_sdma_canwrite_done:  /// CanWriteUpto: done
s_cmp_eq_u32 s36, 0                                // CanWriteUpto == false?
s_cbranch_scc1 label_sdma_reserve_loop             // full -> retry
v_mov_b32 v28, s34                                 // swap lo = new
v_mov_b32 v29, s35                                 // swap hi = new
v_mov_b32 v26, s32                                 // cachedWptr addr lo
v_mov_b32 v27, s33                                 // cachedWptr addr hi
...
v_readfirstlane_b32 s38, v28                       // CAS pre-op lo
s_cmp_eq_u32 s38, s26                              // CAS pre-op lo == cur lo? (won the slot)
s_cbranch_scc0 label_sdma_reserve_loop             // lost race -> retry
v_readfirstlane_b32 s39, v29                       // CAS pre-op hi
s_cmp_eq_u32 s39, s27                              // pre-op hi == cur hi?
s_cbranch_scc0 label_sdma_reserve_loop             // lost race -> retry
...
s_mov_b32 s29, s27                                 // pending = reserved base hi
  s_load_dwordx2 s[32:33], s[22:23], 0x0
s_waitcnt lgkmcnt(0)                               // wait queueBuf pointer load
v_mov_b32 v28, 0                                   // padding NOP value = 0
s_lshr_b32 s34, s30, 2                             // numOffsetDwords = offset / 4
s_cmp_eq_u32 s34, 0                                // no padding?
s_cbranch_scc1 label_sdma_pp_paddone               // offset==0 -> skip pad
label_sdma_pp_padloop:  /// placePacket: zero-pad ring tail
s_and_b32 s31, s28, 262143                         // WrapIntoRing(pending) (pad)
v_mov_b32 v26, s32                                 // queueBuf lo
v_mov_b32 v27, s33                                 // queueBuf hi
v_add_co_u32 v26, vcc, s31, v26                    // addr lo = queueBuf + WrapIntoRing(pending)
...
s_sub_u32 s34, s34, 1                              // numOffsetDwords -= 1
s_cmp_eq_u32 s34, 0                                // pad done?
s_cbranch_scc0 label_sdma_pp_padloop               // more padding
label_sdma_pp_paddone:  /// placePacket: padding done
s_and_b32 s31, s28, 262143                         // WrapIntoRing(pending) (packet base)
v_mov_b32 v26, s32                                 // queueBuf lo
v_mov_b32 v27, s33                                 // queueBuf hi
v_add_co_u32 v26, vcc, s31, v26                    // addr lo = queueBuf + WrapIntoRing(pending)
v_addc_co_u32 v27, vcc, v27, 0, vcc                // addr hi (carry)
...
s_addc_u32 s29, s29, 0                             // pending hi carry
s_mov_b32 s30, 0                                   // ATOMIC follows the COPY: no further padding
  s_load_dwordx2 s[32:33], s[22:23], 0x0
s_waitcnt lgkmcnt(0)                               // wait queueBuf pointer load
v_mov_b32 v28, 0                                   // padding NOP value = 0
s_lshr_b32 s34, s30, 2                             // numOffsetDwords = offset / 4
...
s_addc_u32 s29, s29, 0                             // pending hi carry
  s_load_dwordx2 s[32:33], s[22:23], 0x28
s_waitcnt lgkmcnt(0)                               // wait committedWptr pointer load
v_mov_b32 v26, s32                                 // committedWptr addr lo
v_mov_b32 v27, s33                                 // committedWptr addr hi
...
v_readfirstlane_b32 s31, v28                       // committedWptr lo
s_cmp_eq_u32 s31, s26                              // committedWptr lo == base lo?
s_cbranch_scc0 label_sdma_submit_spin              // not our turn -> spin
v_readfirstlane_b32 s31, v29                       // committedWptr hi
s_cmp_eq_u32 s31, s27                              // committedWptr hi == base hi?
s_cbranch_scc0 label_sdma_submit_spin              // not our turn -> spin
...
// loadKernArg 24 KernArgAddress dword=1 sgprOffset=0x54
s_waitcnt lgkmcnt(0)                               // wait counter_ptr/FusedW
s_mul_i32 s20, s24, s14                            // W * tokenTiles (counter2 base index, past the (p,j) counter)
s_add_u32 s20, s20, s15                            // counter2 index = W*tokenTiles + dst_rank
s_lshl_b32 s20, s20, 2                             // * 4 (u32 counter byte offset)
s_add_u32 s22, s22, s20                            // counter2[dst_rank] lo = counter_ptr + (W*tokenTiles+dst_rank)*4
...
v_readfirstlane_b32 s20, v4                        // old2 -> sgpr
s_add_u32 s20, s20, 1                              // old2 + 1
s_cmp_eq_u32 s20, s14                              // old2+1 == FusedTokenTiles? (this card's last packet to dst_rank)
...
v_readfirstlane_b32 s22, v[vgprSerial]             // wave 0 elects the WG's single tally writer (local path)
s_cmp_eq_u32 s22, 0                                // wave 0?
s_cbranch_scc0 label_fusedA2A_handshake_after      // non-wave-0 -> skip (single writer per WG)
...
s_sub_u32 s23, s[sgprFusedTotalWGs], 1             // wrap limit = FusedTotalWGs - 1 (what the last WG reads back)
s_mov_b32 s22, s23                                 // SDATA in = limit; the atomic overwrites it with the pre-op value
s_atomic_inc s22, s[sgprFusedCounter3Ptr:sgprFusedCounter3Ptr+1], 0 glc // old3 = atomic_inc(counter3), wrap at FusedTotalWGs-1, return pre-op
s_waitcnt lgkmcnt(0)                               // fused-A2A: wait counter3 atomic return (SMEM -> lgkmcnt)
s_cmp_eq_u32 s22, s23                              // pre-op == FusedTotalWGs-1? (globally last WG)
s_cbranch_scc0 label_fusedA2A_handshake_after      // not the last WG -> done (only the globally last WG drains)
// loadKernArg 22 KernArgAddress dword=1 sgprOffset=0x5c
...
s_cmp_eq_u32 s10, 0                                // AM_tiles == 0? (no PUSH WG -> no packet -> no flag)
s_cbranch_scc1 label_fusedA2A_drain_skip           // AM_tiles==0 -> nothing will ever set a flag, skip drain barrier
...
s_lshl_b32 s24, s10, 3                             // rank * 8 (byte offset into peer_ptr[] array)
