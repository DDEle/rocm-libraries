...
//
// The flag buffer's IPC round trip belongs to hipblasLtSetDeviceComm; only the
// recv buffers are the caller's to exchange.
//
...
namespace
{
    ...
    int runProcess(ProcessContext& ctx)
    {
        ...
        for(uint32_t slot = 0; slot < kRanksPerProcess; ++slot)
            if(ctx.ranks[slot].handle != nullptr)
                hipblasLtDestroy(ctx.ranks[slot].handle);
        return status;
    }
}
